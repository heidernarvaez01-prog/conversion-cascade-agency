...
// Blog articles are generated by scripts/build_blog.py (blog/<slug>/index.html).
// They are discovered automatically so a new article is published without
// editing the list above (otherwise it 404s and falls back to the homepage).
const blogDir = path.resolve(templateDir, "blog");
const blogPages = fs.existsSync(blogDir)
  ? fs
      .readdirSync(blogDir, { withFileTypes: true })
      .filter((d) => d.isDirectory() && fs.existsSync(path.join(blogDir, d.name, "index.html")))
      .map((d) => `blog/${d.name}/index.html`)
  : [];
const allPages = [...new Set([...pages, ...blogPages])];
